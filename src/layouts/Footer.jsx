    ...
    <footer className="relative bg-neutral-950 text-white overflow-hidden border-t border-white/5">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="py-16 sm:py-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-4">
            <Link to="/" className="inline-block mb-5">
              <span className="text-3xl font-light tracking-wider text-white">{t.footer.brand}</span>
            </Link>

            <p className="text-gray-500 text-sm leading-relaxed mb-6 max-w-xs italic">{t.footer.slogan}</p>

            {/* Social links */}
            <div className="mb-7">
              <p className="text-xs tracking-[0.2em] uppercase text-pink-400/70 font-medium mb-4">{t.footer.follow}</p>
              <div className="flex flex-wrap gap-2.5">
                {socialLinks.map((social) => (
                  ...
                  >
                    <span className="text-pink-400 transition-transform duration-300 group-hover:scale-110">{social.icon}</span>
          ...
          <div className="lg:col-span-2">
            <h3 className="text-xs tracking-[0.2em] uppercase text-pink-400/70 font-medium mb-5">{t.footer.navigate}</h3>
            <ul className="space-y-3">
              {navLinks.map((link) => (
                <li key={link.key}>
                  <Link to={link.to} className="text-gray-500 hover:text-white text-sm transition-colors duration-200 inline-block">
                    {t.nav[link.key]}
                  </Link>
                </li>
              ))}
              <li>
                <span className="text-gray-500 text-sm inline-block">{t.footer.beautyJournal}</span>
              </li>
            </ul>
          </div>
          ...
          <div className="lg:col-span-3">
            <h3 className="text-xs tracking-[0.2em] uppercase text-pink-400/70 font-medium mb-5">{t.footer.servicesTitle}</h3>
            <ul className="space-y-3">
              {t.footer.services.map((service) => (
                <li key={service}>
                  <span className="text-gray-500 text-sm inline-block">{service}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Visit Us */}
          <div className="lg:col-span-3">
            <h3 className="text-xs tracking-[0.2em] uppercase text-pink-400/70 font-medium mb-5">{t.footer.visitUs}</h3>
            <div className="space-y-4 mb-6">
              ...
              >
                <p className="text-white text-sm font-medium mb-1 group-hover:text-pink-400 transition-colors">{t.footer.address1}</p>
                <p className="text-gray-500 text-xs">{t.footer.address2}</p>
                <p className="text-gray-600 text-[11px] mt-1">{t.footer.address3}</p>
              </a>
              <a
            ...
            <h4 className="text-xs tracking-[0.15em] uppercase text-pink-400/70 font-medium mb-3">{t.footer.hoursTitle}</h4>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t.footer.sunThu}</span>
                <span className="text-gray-300">{t.footer.time1}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t.footer.fri}</span>
                <span className="text-gray-300">{t.footer.time2}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t.footer.sat}</span>
                <span className="text-gray-300">{t.footer.time1}</span>
              </div>
            </div>
        ...
        <div className="border-t border-white/5 py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p className="text-xs text-gray-700">{t.footer.locationStr}</p>
        </div>
      </div>
    </footer>
  );
...
// Added navLinks constant here for mapping since it was in Navbar originally but we need it for the footer links
const navLinks = [
  { to: "/", key: "home" },
  { to: "/about", key: "story" },
