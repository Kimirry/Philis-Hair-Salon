...
export default function Hero() {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-white">
        ...
        <div className="grid min-h-[calc(100vh-7rem)] grid-cols-1 items-center gap-12 py-8 sm:py-10 lg:grid-cols-2 lg:gap-16 xl:gap-20 xl:py-10">
          ...
          <div className="mx-auto w-full max-w-xl text-center lg:mx-0 lg:text-left">
            ...
            <div className="flex items-center justify-center gap-3 lg:justify-start">
              ...
              <span className="text-[10px] font-medium uppercase tracking-[0.3em] text-pink-500 sm:text-xs">{t.hero.eyebrow}</span>
            </div>
            ...
            <h1 className="mt-7 font-serif text-5xl font-medium leading-[0.94] tracking-tight text-gray-900 sm:text-6xl lg:text-[4.4rem] xl:text-[5rem]">
              {t.hero.title1}
              <span className="block text-pink-500">{t.hero.title2}</span>
              {t.hero.title3}
            </h1>

            {/* Location */}
            <p className="mt-7 text-sm uppercase tracking-[0.24em] text-gray-500">{t.hero.location}</p>

            {/* Description */}
            <p className="mx-auto mt-5 max-w-md text-base leading-8 text-gray-500 sm:text-lg lg:mx-0">{t.hero.desc}</p>

            {/* Services */}
            <div className="mt-6 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 lg:justify-start">
              {t.hero.services.map((item, index) => (
                <div key={item} className="flex items-center gap-3">
                  <span className="text-xs text-gray-500 sm:text-sm">{item}</span>
                  {index < 3 && <span className="h-1 w-1 rounded-full bg-pink-300" />}
                </div>
              ))}
            </div>
            ...
            <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row lg:justify-start">
              ...
              >
                {t.hero.bookNow}
                <svg className="ml-2.5 h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div className="mt-8 flex items-center justify-center gap-3 text-xs text-gray-400 lg:justify-start">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-pink-50 text-pink-500">✓</span>
              <span>{t.hero.reassurance}</span>
            </div>
          </div>
          ...
          <div className="flex w-full justify-center lg:justify-end">
            <div className="relative w-full max-w-md xl:max-w-lg">
              ...
              <div className="relative mx-auto aspect-[4/4.6] w-full overflow-hidden rounded-[2rem] bg-pink-50 shadow-[0_25px_70px_rgba(0,0,0,0.10)]">
                <img src={nailsImage} alt={t.hero.imgAlt} className="h-full w-full object-cover object-center transition-transform duration-700 hover:scale-[1.025]" />
      ...
      {/* Discover Philis */}
      <a href="#services" className="absolute bottom-5 left-1/2 hidden -translate-x-1/2 items-center gap-3 text-[9px] uppercase tracking-[0.28em] text-gray-400 transition-colors hover:text-pink-500 lg:flex">
